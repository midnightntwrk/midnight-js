# 0008. Read the network head version on every call
...
We will treat the network head version as an always-fresh read, and answer
read-path era questions from per-record data instead.

Concretely:

- `PublicDataProvider.queryLatestProtocolVersion(): Promise<number>` takes no
  arguments and its contract states that implementations **MUST NOT** cache the
  answer. `IndexerPublicDataProvider` issues one `HEAD_PROTOCOL_VERSION_QUERY`
  with `fetchPolicy: 'no-cache'` per call.
...
- **Positive:** the answer to "which era is the network on" cannot go stale, so
  the next fork (v9 to v10) is observed rather than latched over; a read costs
  exactly the requests the caller asked for, with no corroborating request
  bolted on; the interface loses a five-paragraph MUST-level protocol that every
  future `PublicDataProvider` implementation would have had to reproduce, and
  gains a one-line prohibition instead; the read path's era answer is
  per-record, which is the granularity a fork-spanning history read needs;
  `queryLatestProtocolVersion` means what its name says, at every call.
- **Negative:** the deploy path will pay one extra small request per deploy
  once step 1 is wired; a consumer that polls the head version in a loop pays
  per iteration and must throttle on its own side; and the
...
- **Follow-ups:** wire the deploy-path resolution (steps 1 and 2) with
  ...
  ever become a measured cost, the shape to reach for is the third alternative
  below — an era cache with invalidation — not a return to corroboration.
...
- **Cache the head reading behind a corroboration protocol** (the
  ...
  pre-fork.
- **Cache with a TTL, or memoise within one flow** — rejected for now, on
  YAGNI. It is a defensible shape — bounded staleness, trivial to explain, no
  protocol in the interface — but the cost it would save has not been measured,
  and the only call site that needs the head reading is the rarest write in the
  framework. Nothing rules it out later.
- **Cache the era, with invalidation on a contradicting record** — rejected for
  now, same reason, and recorded here as the shape to prefer if caching is ever
  needed. It caches the monotone quantity (the era, not a head integer, so the
