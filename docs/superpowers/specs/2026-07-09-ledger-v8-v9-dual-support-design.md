...
**Status:** Draft v3.2 (upstream answers applied)
**Date:** 2026-07-09 (v1) · 2026-08-03 (v2 — keep-state rework) · 2026-08-04 (v3 — simplification rework; v3.1 — TS-dev review findings) · 2026-08-05 (v3.2 — upstream answers, #1005)
...
- **OQ1 — RESOLVED (incl. contract status, v3.2).** `protocolVersion` int encodes the **node** version (`major·1_000_000 + minor·1_000`); authoritative mapping in [`midnight-indexer/.../protocol_version.rs`](https://github.com/midnightntwrk/midnight-indexer/blob/main/indexer-common/src/domain/protocol_version.rs): node 0.22 & 1.0 → v8; node 2.0 & 2.1 → v9; anything else → error. Implemented as ranges with a fail-fast else-branch + table test. Convention confirmed (#1005 answer 6) with a nuance: **the protocol major may rise faster than ledger eras, but a ledger era change always requires a protocol major change**. Consequence: the mirrored table must be extended on every node major *even without a fork* — a new-but-same-era int hitting the fail-fast else-branch is the designed maintenance signal, not a bug.
