...
**Status:** Draft v3.10 (transport composition note)
**Date:** 2026-07-09 (v1) · 2026-08-03 (v2 — keep-state rework) · 2026-08-04 (v3 — simplification rework; v3.1 — TS-dev review findings) · 2026-08-05 (v3.2 — upstream answers, #1005; v3.3 — architecture re-review; v3.4 — senior-dev re-review) · 2026-08-06 (v3.5 — per-major version mapping; v3.6 — helpers to `utils`; v3.7 — QA review; v3.8 — version-neutral helper names; v3.9 — @tkerber confirmation, OQ15 re-ruled) · 2026-08-07 (v3.10 — single-request head+state composition)
...
>
> **Revision note (v3.10, 2026-08-07).** Transport note (owner): `networkHeadVersion` is a distinct *source*, not a distinct *roundtrip*. On the call path the provider SHOULD serve `queryLatestProtocolVersion` and `queryRawContractState` from **one composed GraphQL request** (two root fields — client-side query composition, no indexer schema change; the mechanism is an MJS-03 implementation choice: same-tick transport batching or a composed document behind the two interface members). Read paths already cost nothing extra (per-record `protocolVersion`). Benefit beyond the saved roundtrip: the head version and the state bytes derive from a **single indexer snapshot**, removing the head↔state skew between the D9/routing decision and the fetched execution input (complements v3.4 finding 7, which unified the state *consumers* on one fetch). Sourcing semantics unchanged: the head value MUST come from the explicit head field (OQ3d) — a fetched record's own `protocolVersion` is never a substitute; per-operation memoisation, the §6.2 stale-head predicate (its post-rejection re-query is separate by definition) and OQ8's single-source caveat are untouched. Deploys fetch no state and keep the standalone head query.
>
...
1. Fetch the **raw serialized** migrated `ContractState` (v9 envelope) via the provider's raw-state query (§4.4) — bytes, not a WASM object (v3.1 finding 7). **One fetch per operation (v3.4 finding 7):** `contracts` decodes these bytes itself (v9 decode via `protocol/ledger`) for the routing key-set and the SEC-5 pre-check, then hands the **identical bytes** to `bridge.executeCall` — routing, SEC-5 and execution share a single state snapshot. A second independent fetch would open an intra-operation TOCTOU window (routing evaluated against one block, execution input from another — doomed submissions or SEC-5 errors that look like tampering); §6.2's stale-head rule covers only the head-flip race, not this one. The operation's head-version resolution SHOULD ride in the same request (§4.4 single-request composition, v3.10) — head and state then share one indexer snapshot.
