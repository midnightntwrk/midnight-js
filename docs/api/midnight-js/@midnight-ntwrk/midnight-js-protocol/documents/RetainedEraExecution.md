...
[Midnight.js API Reference](../../../packages.md) / [@midnight-ntwrk/midnight-js-protocol](../README.md) / RetainedEraExecution
...
part of the package-wide discipline in [SharedTableDiscipline](SharedTableDiscipline.md).
...
[SharedTableDiscipline](SharedTableDiscipline.md), alongside the same form in `version.ts` and
`loadLedgerEra`.
...
and belongs to [DualInstantiationGuard](DualInstantiationGuard.md).
...
[SharedTableDiscipline](SharedTableDiscipline.md).
...
state carries, and why the address cannot be recomputed — are in [VerifierKeys](VerifierKeys.md)
and [ComposeRefusalOrder](ComposeRefusalOrder.md).
...
composing a call that records nothing, belong to [ComposeRefusalOrder](ComposeRefusalOrder.md).
...
is in [InjectedVendorSlices](InjectedVendorSlices.md).
