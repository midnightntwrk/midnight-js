...
**Status:** Derived from Design Spec Draft **v5.5** (2026-09-04 — re-read against the integration tip of [#1218](https://github.com/midnightntwrk/midnight-js/pull/1218) and the #1006 tree #1204 → #1177 → #1207). **Corrected in this pass:** the Zswap refusal on the retained era is gone (OQ19 closed), and the node-version mapping now covers majors 1 and 2 only. Sections not touched by those two facts still carry their v5.3 derivation.
...
**The fork moment is the primary scenario, not a rare race.** Proving takes minutes, so every in-flight operation at the fork hits a stale head. Detection (QA-3): on submit rejection, re-query the head — a plain uncached read, there being no latch to bypass (ADR 0008) — and raise the stale-head error **iff** `protocolVersionToLedger(head)` differs from the operation-start **era**. Never the raw int: a same-era node minor bump mid-operation must not masquerade as a fork.
...
3. **Fresh re-read on mismatch** — a lagging indexer replica serving a v8-envelope state would otherwise loop on "fork in progress". A mismatch first triggers a fresh head read; only if the fresh head still disagrees is an error raised, and if the fresh head is v9 it is a **distinct indexer-inconsistency error**, never the fork-in-progress text. No bypass flag is involved: since ADR 0008 the head read is uncached anyway, and any permitted cache expires by itself.
...
9. **Breadcrumbs** — every dispatch decision emits a debug-level `loggerProvider` breadcrumb; the head-resolution one carries reading provenance (`fresh|cached`, and on `cached` the age and the bound), without which a lagging-replica incident is indistinguishable from a live read in logs.
...
| D16 | **Never latch the network head version** (reversed at v5.5 — ADR 0008, #1225): no argument, a cache only if it expires by itself, and a read's era taken from the `protocolVersion` that read carries | The superseded monotone latch saved a request mid-era and misreported at the era boundary — upgrade-only means nothing clears it at the next fork; it was inert pre-fork by its own contract; and its corroboration route cost an extra head request per finalized transaction until it engaged |
...
flowchart LR
    ...
    s3 --> s4["4. providers (MJS-03)<br/>dual decode, both queries,<br/>version-tagged flows"]
