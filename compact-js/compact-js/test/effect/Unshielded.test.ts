...
import {
  ...
  Contract,
  ContractExecutable,
  ContractExecutableRuntime} from '@midnight-ntwrk/compact-js/effect';
...
describe('Unshielded Tokens', () => {
  ...
  beforeEach(async () => {
    ...
    domainSep = Arbitrary.singleSample(Arbitrary.makeDomainSeparatorArbitrary());
  });

  describe('minting', () => {
    it.effect('should return unshielded spends for a given contract address', () => Effect.gen(function* () {
      const address = Arbitrary.singleSample(Arbitrary.makeContractAddressArbitrary());
    ...
    it.effect('should return unshielded spends from the deployed contract when minting', () => Effect.gen(function* () {
      const mintResult = yield* runtime.runFork(contract.circuit(
