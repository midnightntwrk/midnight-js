...
| `MIDNIGHT_JS_C_ERA_INVARIANT_VIOLATION` | A provider returned a payload from a ledger era other than the one this flow submits. `seam` names the seam, `expected` the era required, and `circuitId` the entry point when one applies. | Check that the configured provider matches the network this application targets, and that no custom provider implementation re-tags the payload it was handed. |
