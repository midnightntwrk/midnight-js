...
export const getValidZKConfig = async () => ({
  ...
  proverKey: createProverKey(
    await fs.readFile(`${resourceDir}/keys/${SET_TOPIC_CIRCUIT_ID}.prover`)
  ),
  verifierKey: createVerifierKey(
    await fs.readFile(`${resourceDir}/keys/${SET_TOPIC_CIRCUIT_ID}.verifier`)
  ),
  zkir: createZKIR(await fs.readFile(`${resourceDir}/zkir/${SET_TOPIC_CIRCUIT_ID}.bzkir`))
});

export const getValidUnprovenTx = async () =>
  UnprovenTransaction.deserialize(
    await fs.readFile(`${resourceDir}/${UNPROVEN_TX}`),
    getLedgerNetworkId()
  );
