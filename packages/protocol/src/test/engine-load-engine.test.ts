...
//
// Ungated: createLedger8Engine imports `compact-runtime-ledger8` and
// `@midnight-ntwrk/onchain-runtime-v3` as bare node_modules specifiers, so
// nothing here resolves through dist/.
describe('createLedger8Engine', () => {
  it('exposes exactly the four retained-era methods, and nothing else', async () => {
    const engine = await createLedger8Engine();

    // Strict equality, not four typeof checks: a fifth method leaking the
    // captured 0.16 runtime or v8 module out of the closure would defeat the
    // point of the facade, and a subset assertion would not notice.
