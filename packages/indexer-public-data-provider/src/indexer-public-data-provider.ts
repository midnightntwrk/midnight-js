...
const maybeThrowGraphQLErrors = <A, R extends FetchResult<A> | ApolloQueryResult<A>>(
  result: R
): R => {
...
const blockOffsetToBlock$ =
  (apolloClient: ApolloClient<NormalizedCacheObject>) => (offset: InputMaybe<BlockOffset>) =>
    zenToRx(
      apolloClient
        .subscribe({
...
const indexerPublicDataProviderInternal = (
  ...
  return {
    ...
    async queryDeployContractState(
      contractAddress: ContractAddress
    ): Promise<ContractState | null> {
      return apolloClient
        .query({
        ...
        .then((queryResult) => {
          if (queryResult.data.contractAction) {
            ...
            return 'deploy' in contract
              ? contract.deploy.transaction.contractActions.find(
        ...
        .then((maybeContractState) =>
          maybeContractState ? deserializeContractState(maybeContractState) : null
        );
    },
    async watchForContractState(contractAddress: ContractAddress): Promise<ContractState> {
      return Rx.firstValueFrom(
        waitForContractToAppear(apolloClient)(contractAddress)(null).pipe(
    ...
    contractStateObservable(
      ...
      const offset =
        config.type === 'blockHash' ? { hash: config.blockHash } : { height: config.blockHeight };
...
export const indexerPublicDataProvider = (
   ...
   */
  const publicDataProvider = indexerPublicDataProviderInternal(
    queryURL,
    subscriptionURL,
    webSocketImpl
  );
  return {
    contractStateObservable(
      ...
      return publicDataProvider.contractStateObservable(
        prependNetworkIdHex(contractAddress),
        config
      );
    },
    queryContractState(
    ...
    queryZSwapAndContractState(
      ...
      assertIsContractAddress(contractAddress);
      return publicDataProvider.queryZSwapAndContractState(
        prependNetworkIdHex(contractAddress),
        config
      );
    },
