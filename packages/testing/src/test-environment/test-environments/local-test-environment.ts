...
export class LocalTestEnvironment extends TestEnvironment {
  ...
  startWithInjectedEnvironment = async (
    ...
    this.logger.info(
      `Test environment configuration: ${JSON.stringify(this.environmentConfiguration)}`
    );
  ...
  start = async (maybeProofServerContainer?: ProofServerContainer) => {
    ...
    this.dockerEnv = await new DockerComposeEnvironment(this.config.path, this.config.fileName)
      ...
      .withWaitStrategy(
        `${this.config.container.node.name}_${this.uid}`,
        this.config.container.node.waitStrategy
      )
      .withWaitStrategy(
        `${this.config.container.indexer.name}_${this.uid}`,
        this.config.container.indexer.waitStrategy
      )
      .withEnvironment({
  ...
  startMidnightWalletProviders = async (
    ...
    this.walletProviders = await Promise.all(
      Array.from({ length: amount }).map((_elem, index) =>
        MidnightWalletProvider.build(
          this.logger,
          this.environmentConfiguration,
          this.genesisMintWalletSeed[index]
        )
      )
    );
