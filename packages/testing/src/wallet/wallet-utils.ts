 ...
 */
export const waitForFullSync = (wallet: Wallet, throttleTime: number = 3_000) => {
 ...
 */
export const waitForSyncProgressDefined = async (wallet: Wallet, throttleTime: number = 3_000) => {
 ...
 */
export const syncWallet = (wallet: Wallet, throttleTime: number = 3_000) => {
  ...
  return Rx.firstValueFrom(
    wallet.state().pipe(
      ...
      Rx.tap((state) => logger.info(logState(state))),
      Rx.filter(
        (state) => state.syncProgress !== undefined && state.syncProgress.lag.applyGap < 100n
      ),
 ...
 */
export const waitForPending = (wallet: Wallet, throttleTime: number = 1_000) => {
 ...
 */
export const waitForFinalizedBalance = (wallet: Wallet, throttleTime: number = 5_000) => {
 ...
 */
export const normalizeWalletState = (state: WalletState): { [key: string]: unknown } => {
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
export const validateFinalState = (
  ...
  expect(finalState.transactionHistory.length).toBeGreaterThanOrEqual(
    initialState.transactionHistory.length + 1
  );
...
export const sendTransactionToAddress = async (
  ...
  address: string,
  outputValue: bigint = 100_000_000n
): Promise<void> => {
 ...
 * @returns {Promise<void>}
 */
export const sendTransactionToWallet = async (
  walletWithFunds: MidnightWallet,
  destination: MidnightWallet
) => {
