...
export class WalletFactory {
  ...
  static buildFromSeedAndTryToRestoreState = async (
    ...
    const restoreWalletFromFile = async () => {
      ...
      try {
        const serialized = await new WalletSaveStateProvider(
          logger,
          seed,
          directoryPath,
          `${filename}`
    ...
    if (directoryPath != null && fs.existsSync(`${directoryPath}/${filename}`)) {
      ...
      logger.info(
        directoryPath != null ? 'Wallet save file not found' : 'File path for save file not found'
      );
    }
