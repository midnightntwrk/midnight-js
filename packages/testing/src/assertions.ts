...
export const expectFoundAndDeployedTxPublicDataEqual = <C extends Contract>(
  ...
  expect(
    stateValueEqual(
      deployTxData.public.initialContractState.data,
      foundDeployTxData.public.initialContractState.data
    )
  ).toBeTruthy();
...
export const expectFoundAndDeployedTxPrivateDataEqual = <C extends Contract>(
  ...
  expect(deployTxData.private.initialPrivateState).toEqual(
    foundDeployTxData.private.initialPrivateState
  );
};
...
export const expectFoundAndDeployedStatesEqual = async <C extends Contract>(
  ...
  expect(
    stateValueEqual(deployedLedgerState!.data, foundDeployTxData.public.initialContractState.data)
  ).toBeTruthy();
  if (privateStateId) {
...
export const expectSuccessfulDeployTx = async <C extends Contract>(
  ...
  if (deployTxOptions) {
    if (deployTxOptions.signingKey) {
      ...
      const storedSigningKey = await providers.privateStateProvider.getSigningKey(
        deployTxData.public.contractAddress
      );
    ...
    if ('privateStateId' in deployTxOptions && 'initialPrivateState' in deployTxOptions) {
      ...
      const storedPrivateState = await providers.privateStateProvider.get(
        deployTxOptions.privateStateId
      );
