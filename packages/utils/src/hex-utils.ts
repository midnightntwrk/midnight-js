...
export function assertIsHex(
  source: string,
  byteLen?: number
): asserts source is NonNullable<string> {
  if (!source) {
  ...
  if (byteLen) {
    ...
    if (byteLen !== actualByteLen) {
      throw new TypeError(
        `Expected an input string with byte length of ${byteLen}, got ${actualByteLen}.`
      );
    }
  }
}
 ...
 */
export const parseCoinPublicKeyToHex = (
  possibleBech32: string,
  zswapNetworkId: NetworkId
): string => {
 ...
 */
export const parseEncPublicKeyToHex = (
  possibleBech32: string,
  zswapNetworkId: NetworkId
): string => {
  if (isHex(possibleBech32)) return possibleBech32;
