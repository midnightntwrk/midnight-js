...
describe('Hex Utils', () => {
  describe('with valid strings', () => {
    ...
    it.each([THREE_BYTE_STRING, `0x${THREE_BYTE_STRING}`])(
      'return true for any byte length',
      (source) => {
    ...
    it.each([THREE_BYTE_STRING, `0x${THREE_BYTE_STRING}`])(
      'return false with a zero byte length',
      (source) => {
        expect(isHex(source, 0)).toBeFalsy();
        expect(() => assertIsHex(source, 0)).toThrow(
          'Expected byte length must be greater than zero.'
        );
      }
    );
    ...
    it('return false when string is not of expected length', () => {
      ...
      expect(() => assertIsHex(THREE_BYTE_STRING, 5)).toThrow(
        `Expected an input string with byte length of ${byteLength}, got ${
          THREE_BYTE_STRING.length / 2
        }.`
      );
    });
  });
  ...
  describe('with invalid strings', () => {
    ...
    it.each([
      ...
      expect(() => assertIsHex(source)).toThrow(
        `Invalid hex-digit 'H' found in input string at index ${invalidPos}.`
      );
    });
    ...
    it.each([INCOMPLETE_BYTE_STRING, `0x${INCOMPLETE_BYTE_STRING}`])(
      ...
      (source) => {
        expect(isHex(source)).toBeFalsy();
        expect(() => assertIsHex(source)).toThrow(
          `The last byte of input string '${source}' is incomplete.`
        );
      }
    );
  });
  ...
  describe('with empty strings', () => {
    ...
    it.each([undefined, 2])('return false regardless of byte length', (byteLen) => {
      ...
      expect(() => assertIsHex(EMPTY_STRING, byteLen)).toThrow(
        'Input string must have non-zero length.'
      );
    });
    ...
    it.each([undefined, 2])('return false regardless of byte length and prefix', (byteLen) => {
      ...
      expect(() => assertIsHex(source, byteLen)).toThrow(
        `Input string '${source}' is not a valid hex-string.`
      );
    });
  });
  ...
  describe('parseCoinPublicKeyToHex', () => {
    ...
    describe('with valid inputs', () => {
      ...
      it('should parse a valid Bech32m-encoded public key and return its hex representation', () => {
        const bech32Input =
          'mn_shield-cpk_undeployed1mjngjmnlutcq50trhcsk3hugvt9wyjnhq3c7prryd5nqmvtzva0sn7kq7h';
  ...
  describe('parseEncPublicKeyToHex', () => {
    ...
    describe('with valid inputs', () => {
      ...
      it('should parse a valid Bech32m-encoded public key and return its hex representation', () => {
        const bech32Input =
          'mn_shield-epk_test1qvqpla2ttj8v49y52ayluya23spy3m6dgt3mxv9sp9cduuq8uw2lw0snz2sze';
