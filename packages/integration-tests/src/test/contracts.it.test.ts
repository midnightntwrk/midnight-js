...
import { CounterClonePrivateStateId, type CounterCloneCircuits } from '../counter-clone-types';
...
describe('Contracts API', () => {
  ...
  it('should create unproven call and deploy transactions for contract with private state', async () => {
    ...
    expect(Counter.ledger(unprovenDeployTxResult.public.initialContractState.data).round).toEqual(
      0n
    );
  ...
  it('should throw error if contract address has wrong format', async () => {
    await expect(
      ...
      `The last byte of input string '${INVALID_CONTRACT_ADDRESS_HEX_FORMAT}' is incomplete.`
    );
  });
