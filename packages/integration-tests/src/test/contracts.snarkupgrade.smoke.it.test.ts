...
import { cloneContractInstance, CounterCloneConfiguration, counterContractInstance } from '../api';
...
import { CIRCUIT_ID_RESET, VERY_SLOW_TEST_TIMEOUT } from '../constants';
...
const logger = createLogger(
  path.resolve(
    ...
    'logs',
    'tests',
    `contracts_snark_upgrade_${new Date().toISOString()}.log`
  )
);
...
describe('Contracts API Snark Upgrade [@slow][@smoke]', () => {
  ...
  it(
    ...
    async () => {
      ...
      const finalizedTxDataIncrement =
        await circuitMaintenanceTxInterfaces.increment.removeVerifierKey();
      const finalizedTxDataDecrement =
        await circuitMaintenanceTxInterfaces.decrement.removeVerifierKey();
      ...
      const vkIncrement =
        await counterCloneContractProviders.zkConfigProvider.getVerifierKey('increment');
      const vkDecrement =
        await counterCloneContractProviders.zkConfigProvider.getVerifierKey('decrement');

      const finalizedTxDataReset2 =
        await circuitMaintenanceTxInterfaces.reset.insertVerifierKey(vkReset);
      const finalizedTxDataIncrement2 =
        await circuitMaintenanceTxInterfaces.increment.insertVerifierKey(vkIncrement);
      const finalizedTxDataDecrement2 =
        await circuitMaintenanceTxInterfaces.decrement.insertVerifierKey(vkDecrement);
  ...
  it(
    ...
    async () => {
      const oldAuthority =
        await counterProviders.privateStateProvider.getSigningKey(contractAddress);
      expect(oldAuthority).not.toBeNull();

      if (oldAuthority) {
        const newAuthority = sampleSigningKey();
        const finalizedTxData = await submitReplaceAuthorityTx(
          counterProviders,
          contractAddress
        )(newAuthority);
        ...
        await expect(
          submitRemoveVerifierKeyTx(counterProviders, contractAddress, CIRCUIT_ID_RESET)
