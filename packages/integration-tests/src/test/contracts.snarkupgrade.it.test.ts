...
import { createLogger, getTestEnvironment } from '@midnight-ntwrk/midnight-js-testing';
import path from 'path';
import * as api from '../api';
import { counterContractInstance } from '../api';
import { type CounterProviders } from '../counter-types';
import { CIRCUIT_ID_RESET } from '../constants';
...
describe('Contracts API Snark Upgrade [dedicated contract] [@slow]', () => {
  ...
  beforeEach(async () => {
    ...
    ({ counterProviders, contractAddress } = await api.deployCounterContract(
      wallet,
      environmentConfiguration
    ));
  });
  ...
  it('should successfully remove verifier key using submitRemoveVerifierKeyTx', async () => {
    const finalizedTxData = await submitRemoveVerifierKeyTx(
      counterProviders,
      contractAddress,
      CIRCUIT_ID_RESET
    );
  ...
  it('should fail when inserting verifier key for wrong circuit after removal', async () => {
    ...
    await expect(() =>
      circuitMaintenanceTxInterfaces.increment.insertVerifierKey(vk)
    ).rejects.toThrow(
