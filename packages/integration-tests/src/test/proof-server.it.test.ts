...
import { ZswapChainState } from '@midnight-ntwrk/zswap';
import {
  ContractCall,
  ContractDeploy,
  sampleCoinPublicKey,
...
import { CounterConfiguration, counterContractInstance } from '../api';
...
describe('Proof server integration', () => {
  ...
  beforeAll(async () => {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<CounterCircuits>(
      new CounterConfiguration().zkConfigPath
    );
    ...
    const verifierKeys = await zkConfigProvider.getVerifierKeys(
      getImpureCircuitIds(counterContractInstance)
    );
  ...
  test('should throw error for invalid ZKConfig circuitId', async () => {
    ...
    await expect(
      proofProvider.proveTx(unprovenCallTx, { zkConfig: invalidZkConfig })
