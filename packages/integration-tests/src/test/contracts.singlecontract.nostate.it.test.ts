...
const logger = createLogger(
  path.resolve(
    ...
    'logs',
    'tests',
    `contracts_nostate_${new Date().toISOString()}.log`
  )
);
...
const expectSimpleContractCallResult = (
  ...
  expect(callResult.private.nextZswapLocalState).toEqual(
    decodeZswapLocalState(
      emptyZswapLocalState(parseCoinPublicKeyToHex(coinPublicKey, getZswapNetworkId()))
    )
  );
...
describe('Contracts API', () => {
  ...
  it('should deploy and find contracts with no private state [@slow]', async () => {
    ...
    await expect(
      findDeployedContract(providers, expandedFindDeployedContractConfig)
    ).rejects.toThrow(
