...
import { CounterCloneConfiguration, counterContractInstance, SimpleConfiguration } from '../api';
import { type CounterProviders, type DeployedCounterContract } from '../counter-types';
import { CIRCUIT_ID_DECREMENT, CIRCUIT_ID_RESET, UNDEPLOYED_CONTRACT_ADDRESS } from '../constants';
...
describe('Contracts API Snark Upgrade [single contract]', () => {
  ...
  beforeAll(async () => {
    ...
    simpleContractProviders = initializeMidnightProviders(
      wallet,
      environmentConfiguration,
      new SimpleConfiguration()
    );
    ({ counterProviders, deployedCounterContract, contractAddress } =
      await api.deployCounterContract(wallet, environmentConfiguration));
  });
  ...
  it('submitReplaceAuthorityTx - should fail on replace contract that is not deployed to contract address', async () => {
    ...
    await expect(
      submitReplaceAuthorityTx(counterProviders, UNDEPLOYED_CONTRACT_ADDRESS)(authority)
    ).rejects.toThrow(
  ...
  it('submitReplaceAuthorityTx - should fail when signing key for contract address does not exist', async () => {
    ...
    await expect(
      submitReplaceAuthorityTx(counterCloneContractProviders, contractAddress)(authority)
    ).rejects.toThrow(`Signing key for contract address '${contractAddress}' not found`);
  });
  ...
  it('submitInsertVerifierKeyTx - should fail on invalid verifier key', async () => {
    ...
    await expect(
      submitInsertVerifierKeyTx(counterProviders, contractAddress, CIRCUIT_ID_RESET, vk)
    ).rejects.toThrow(
  ...
  it('submitRemoveVerifierKeyTx - should fail on not present circuitId', async () => {
    ...
    await expect(
      submitRemoveVerifierKeyTx(counterProviders, contractAddress, circuitId)
    ).rejects.toThrow(
  ...
  it('submitRemoveVerifierKeyTx - should fail on providers for different contract with different API ', async () => {
    await expect(
      submitRemoveVerifierKeyTx(simpleContractProviders, contractAddress, CIRCUIT_ID_RESET)
    ).rejects.toThrow(`Signing key for contract address '${contractAddress}' not found`);
  });
  ...
  it('createContractMaintenanceTxInterface - replaceAuthority - successful replace authority with the new one [@slow]', async () => {
    ...
    const finalizedTxData1 = await submitRemoveVerifierKeyTx(
      counterProviders,
      contractAddress,
