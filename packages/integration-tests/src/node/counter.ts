...
import { CounterPrivateStateId, type CounterProviders, privateStateZero } from '../counter-types';
...
async function counter() {
  ...
  let contractConfiguration: api.CounterConfiguration;

  logger.info('Starting counter...');
  try {
    testEnvironment = getTestEnvironment(logger);
    environmentConfiguration = await testEnvironment.start();
    contractConfiguration = new api.CounterConfiguration();
    ...
    await api.getCounterLedgerState(
      providers,
      deployedCounterContract.deployTxData.public.contractAddress
    );
