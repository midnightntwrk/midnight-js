 ...
 */
export type DeployTxOptionsWithPrivateStateId<C extends Contract> =
 ...
 */
export type UnprovenDeployTxOptions<C extends Contract> =
...
export async function createUnprovenDeployTx<C extends Contract>(
  ...
  const verifierKeys = await providers.zkConfigProvider.getVerifierKeys(
    getImpureCircuitIds(options.contract)
  );
