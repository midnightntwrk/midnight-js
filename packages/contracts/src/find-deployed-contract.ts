...
/**
 * The catch-all arm, and deliberately the LAST overload of `findDeployedContract`.
 *
 ...
 * below it.
 *
 * Its declared return type RESTATES the store-private-state arm above it, instantiated at that
 ...
 * a `never` here silently retypes `ReturnType<typeof findDeployedContract>` for every existing
 ...
 * arm above.
 */
export async function findDeployedContract<P, T extends NeitherEraContractOptions>(
  providers: P,
  options: T
): Promise<FoundContract<Contract.Any>>;
