...
const setOrGetInitialPrivateState = async <C extends Contract>(
  ...
  if (hasPrivateStateId) {
    ...
    assertDefined(
      currentPrivateState,
      `No private state found at private state ID '${options.privateStateId}'`
    );
 ...
 * private state ID.
 */
export type FindDeployedContractOptionsExistingPrivateState<C extends Contract> =
...
export async function findDeployedContract<C extends Contract>(
  ...
  const initialContractState =
    await providers.publicDataProvider.queryDeployContractState(contractAddress);
  assertDefined(
  ...
  const initialPrivateState = await setOrGetInitialPrivateState(
