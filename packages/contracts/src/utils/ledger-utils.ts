...
export const toLedgerContractState = (contractState: ContractState): LedgerContractState =>
  LedgerContractState.deserialize(
    contractState.serialize(getRuntimeNetworkId()),
    getLedgerNetworkId()
  );
...
export const toLedgerQueryContext = (queryContext: QueryContext): LedgerQueryContext =>
  new LedgerQueryContext(
    LedgerStateValue.decode(queryContext.state.encode()),
    queryContext.address
  );

const addVerifierKeys = (
...
export const createUnprovenLedgerCallTx = (
  ...
  assertDefined(
    op,
    `Operation '${circuitId}' is undefined for contract state ${initialContractState.toString(
      false
    )}`
  );
...
export const unprovenTxFromContractUpdates = (
  ...
  const signedMaintenanceUpdate = maintenanceUpdate.addSignature(
    idx,
    signData(sk, maintenanceUpdate.dataToSign)
  );
  return new UnprovenTransaction(
    new UnprovenOffer(),
    undefined,
