...
const toOutputData = (recipient: Recipient, coinInfos: (QualifiedCoinInfo | CoinInfo)[]) =>
  coinInfos.map((coinInfo) =>
    'mt_index' in coinInfo
      ? { recipient, coinInfo: dropMtIndex(coinInfo) }
      : { recipient, coinInfo }
  );
...
describe('Zswap utilities', () => {
  ...
  const zswapChainStateWithNonMatchingInputs = (recipient: Recipient, values: bigint[]) => {
    ...
    const zswapChainState = values.reduce((prevZSwapChainState, value) => {
      ...
      if (proofErasedOffer) {
        const [newZswapChainState, mtIndices] =
          prevZSwapChainState.tryApplyProofErased(proofErasedOffer);
  ...
  const arbitraryMatchingInputOutputPairs = (
    ...
    fc
      .array(arbitraryNativeCoinInfo.filter(distinctFrom(preExistingCoins)))
  ...
  const arbitraryZswapScenario = fc
    ...
    .chain(([recipient, values]) => {
      const { nonMatchingInputs, zswapChainState } = zswapChainStateWithNonMatchingInputs(
        recipient,
        values
      );
      return fc
        ...
        .chain((nonMatchingOutputsNoRecipient) =>
          arbitraryMatchingInputOutputPairs(
            recipient,
            nonMatchingOutputsNoRecipient.concat(nonMatchingInputs)
    ...
    const arbitraryScenario = arbitraryCoinPublicKey.chain((walletCoinPublicKey) =>
      fc.record<ScenarioData>({
        ...
        outputsNotForWallet: fc
          ...
          .map((coinsAndRecipients) =>
            coinsAndRecipients.map(([coinInfo, recipient]) => ({ coinInfo, recipient }))
          )
      })
    );
