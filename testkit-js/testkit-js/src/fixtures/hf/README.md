...
here against the real `@midnightntwrk/ledger-v8@8.1.2` /
...
`COMPACTC_VERSION=0.34.0`, and root `resolutions` pins
`@midnight-ntwrk/compact-runtime` to `0.19.0` — see "Version pin note"
...
| `private-counter-twin/compiled/` | 0.34.0 | 0.26.0 | **0.19.0** | async (current) |
...
```
compact list          # 0.31.1 must be installed, for the retained side
# The retained side comes from a `compact`-installed toolchain; the current side
# comes from the one THIS REPO pins (`.envrc`'s COMPACTC_VERSION), because that
# is the version whose `runtime-version` the consuming test gates on. Fetch it
# once, from packages/contracts/, if packages/compact/managed/ has no such
# directory yet:
#   yarn fetch-compactc
# The `*` is the platform triple `compact` installs under.
...
packages/compact/managed/0.34.0/compactc       --skip-zk $SRC /tmp/private-counter-cur
...
resolves; the repo-pinned `0.34.0` is the one that emits `0.19.0` today, and the
...
dependencies here — only `@midnightntwrk/ledger-v8@8.1.2` and
