...
for (const entry of RETAINED_MATRIX.filter((candidate) => covers(SELECTED.retained, candidate.key))) {
  await probe(`a migrated ${entry.key}, called a second time`, async () => {
    ...
    // The first post-fork call's own circuit and arguments, and nothing else:
    // this probe is about whether the call is ADMITTED, so it carries no
    // assertion on the return value -- one would turn an answer into a failure.
    const [{ circuitId, args }] = [entry.postFork].flat();
    const context = await walletContext(session.wallet);

    try {
      const outcome = await callRetained(entry.key, providers, deployed.contractAddress, { circuitId, args }, context);
